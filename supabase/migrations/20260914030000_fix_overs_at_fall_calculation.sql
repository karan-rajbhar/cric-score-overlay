-- ============================================================================
-- FIX OVERS AT FALL & PARTNERSHIP OVERS CALCULATION
-- Replace ROUND(v_legal / 10, 1) with proper 6-ball cricket overs:
-- ROUND(FLOOR(v_legal / 6.0)::numeric + ((v_legal % 6)::numeric / 10), 1)
-- ============================================================================
    ...
    BEGIN
      IF v_ball.bowler_id IS NOT NULL THEN
        INSERT INTO public.bowling_performances
          (match_id, innings_id, user_id, bowling_position)
        VALUES
          (v_match_id, p_innings_id, v_ball.bowler_id,
           (SELECT COALESCE(MAX(bowling_position), 0) + 1 FROM public.bowling_performances WHERE innings_id = p_innings_id))
        ON CONFLICT (innings_id, user_id) DO NOTHING;
      END IF;
    END;
