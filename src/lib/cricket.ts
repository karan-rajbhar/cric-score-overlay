...
export const ballsFromOvers = (overs: number): number =>
    Math.round(overs) * 6 + Math.round((overs - Math.floor(overs)) * 10);

/** Balls bowled by a bowler row, tolerating either engine column. */
export const bowlerBalls = (b: Pick<BowlingPerformance, "balls_bowled" | "overs_bowled">): number =>
    (b.balls_bowled ?? 0) > 0 ? b.balls_bowled : ballsFromOvers(b.overs_bowled ?? 0);

export const hasBowled = (b: Pick<BowlingPerformance, "balls_bowled" | "overs_bowled">): boolean =>
    (b.balls_bowled ?? 0) > 0 || (b.overs_bowled ?? 0) > 0;

export const strikeRate = (runs: number | null | undefined, balls: number | null | undefined): string =>
    balls && balls > 0 ? ((runs ?? 0) / balls * 100).toFixed(1) : "—";

export const economyRate = (runs: number | null | undefined, balls: number | null | undefined): string =>
...
export const inningsForTeam = (match: Match, teamId: string): Innings | undefined =>
    match.innings?.find((i) => i.team_id === teamId);

/** Innings in which a team bowled (i.e. the opponent's innings). */
export const bowlingInningsForTeam = (match: Match, teamId: string): Innings | undefined =>
    match.innings?.find((i) => i.team_id !== teamId);

/** Top batters of a team, best first. */
export function topBatters(innings: Innings | undefined, limit = 3): BattingPerformance[] {
...
export function topBowlers(match: Match, teamId: string, limit = 3): BowlingPerformance[] {
