...
export function ScoringPanel({
    ...
    const extraTypes = [
        ...
        { type: "no_ball", label: "No ball", activeClass: "bg-orange-500 text-white" },
        { type: "bye", label: "Bye", activeClass: "bg-sky-600 text-white" },
        { type: "leg_bye", label: "Leg bye", activeClass: "bg-violet-500 text-white" },
    ];

    const handleRunClick = (runs: number) => {
        if (selectedExtra) {
            // Extra + additional runs (e.g. wide + 1 boundary).
            onScore(runs === 0 ? 0 : runs, { type: selectedExtra, runs: runs === 0 ? 1 : runs });
    ...
    const getBallDisplay = (ball: string) => {
        ...
        if (ball === "4") return "bg-sky-500/15 text-sky-600 dark:text-sky-400 border-sky-500/40";
        if (ball === "6") return "bg-violet-500/15 text-violet-600 dark:text-violet-400 border-violet-500/40";
            ...
            <CardContent className="space-y-4 p-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-md border border-border bg-secondary px-2.5 py-1 font-score text-lg font-semibold leading-none tabular">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {lastBalls.length > 0 ? (
                        <div className="flex gap-1.5">
                            {lastBalls.map((ball, idx) => (
                                <span
                                    key={idx}
                                    className={cn(
                                        "inline-flex h-7 min-w-7 items-center justify-center rounded-full border px-1.5 text-xs font-semibold tabular",
                                        getBallDisplay(ball)
                                    )}
                                >
                                    {ball}
                        ...
                        <span className="text-xs text-muted-foreground/70">No deliveries yet</span>
                    )}
                </div>
                ...
                {selectedExtra && (
                    <div className="flex items-center justify-between rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => setSelectedExtra(null)}>
                            <X className="h-4 w-4" />
                        </Button>
                    </div>
                )}
