...
export function useScoring(matchId: string) {
    ...
    const loadPlayers = async (matchData: Match) => {
        ...
        const bowlingTeamId =
            battingTeamId === matchData.team1_id ? matchData.team2_id : matchData.team1_id;
        const [battingResult, bowlingResult] = await Promise.all([
            getTeamPlayers(battingTeamId),
            getTeamPlayers(bowlingTeamId),
        ]);
        if (battingResult.data) setBattingTeamPlayers(battingResult.data as Player[]);
        if (bowlingResult.data) setBowlingTeamPlayers(bowlingResult.data as Player[]);
    };

    const syncFromDb = useCallback(async () => {
        const result = await getScoringState(matchId);
        if (result.error || !result.data) return;
        const { match: m, strikerId: s, nonStrikerId: ns, bowlerId: b, thisOverDeliveries } =
            result.data as {
                match: Match;
    ...
    const applyState = (state: ScoringState) => {
        ...
        setLastBalls((prev) => [...prev, deliveryLabel(state as unknown as { runs_scored: number; extras: number; extra_type: string; is_wicket: boolean })].slice(-6));
        void syncFromDb();
    };

    const handleStartMatch = async (tossWinner: string, tossDecision: "bat" | "bowl") => {
    ...
    const handleConfirmBatsmen = async (sId: string | null, nsId: string | null) => {
    ...
    const handleAddPlayerInline = async (
        ...
        const battingTeamId = currentInnings?.team_id;
        const bowlingTeamId =
            battingTeamId === currentMatch.team1_id ? currentMatch.team2_id : currentMatch.team1_id;
