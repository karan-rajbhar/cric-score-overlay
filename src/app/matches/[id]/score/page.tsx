...
import { ChevronLeft, Tv, Loader2, AlertCircle, Play, Undo2, UserPlus, Users } from "lucide-react";
...
export default function ScoringPage() {
    ...
    const [addPlayerTeam, setAddPlayerTeam] = useState<"batting" | "bowling">("batting");
    ...
    const currentInnings = match?.innings?.find((i) => i.innings_number === match?.current_innings);
    const battingTeam = currentInnings?.team_id === match?.team1_id ? match?.team1 : match?.team2;
    const bowlingTeam = currentInnings?.team_id === match?.team1_id ? match?.team2 : match?.team1;
    ...
    const onScore = async (runs: number, extra?: { type: string; runs: number }) => {
    ...
    const onWicket = async () => {
        if (!strikerId) return;
        const needsFielder = DISMISSAL_TYPES.find((d) => d.value === dismissalType)?.needsFielder;
    ...
    const onAddPlayer = async () => {
        ...
        await handleAddPlayerInline(addPlayerTarget, newPlayerName, match, strikerId, nonStrikerId, currentBowlerId);
        setNewPlayerName("");
    ...
    const onAddPlayerAlways = async () => {
        ...
        await handleAddPlayerInline(addPlayerTeam, newPlayerName, match, strikerId, nonStrikerId, currentBowlerId);
        setNewPlayerName("");
    ...
    const isScorer = !!user && (match.created_by === user.id || (match.match_admins ?? []).includes(user.id));
                    ...
                    <Card>
                        <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-muted-foreground">{battingTeam?.name ?? "—"} batting</p>
                                    <p className="score-display text-3xl font-bold">
                                        {currentInnings?.total_runs ?? 0}/{currentInnings?.total_wickets ?? 0}
                                        <span className="ml-2 text-lg font-normal text-muted-foreground">
                                            ({currentInnings ? `${Math.floor((currentInnings.total_balls ?? 0) / 6)}.${(currentInnings.total_balls ?? 0) % 6}` : "0.0"} ov)
                                        </span>
                                    </p>
                                </div>
                                <div className="flex gap-2">
                                    <Button variant="outline" size="sm" onClick={() => handleUndo()} disabled={isProcessing}>
                                        <Undo2 className="mr-1.5 h-4 w-4" /> Undo
                                    </Button>
                                    <Button variant="outline" size="sm" onClick={() => handleEndInnings()} disabled={isProcessing}>
                                        End Inns
                                    </Button>
                                </div>
                            </div>
                            {lastBalls.length > 0 && (
                                <div className="mt-3 flex flex-wrap gap-1.5">
                                    {lastBalls.map((b, i) => (
                                        <Badge key={i} variant={b === "W" ? "destructive" : "secondary"} className="tabular">
                                            {b}
                                        </Badge>
                                    ))}
                                </div>
                    ...
                    <CurrentBatsmen
                        batsman1={
                            battingTeamPlayers.find((p) => p.user_id === strikerId)
                                ? {
                                      id: strikerId!,
                                      name: battingTeamPlayers.find((p) => p.user_id === strikerId)!.user!.full_name,
                                      runs: 0,
                                      balls: 0,
                                      fours: 0,
                                      sixes: 0,
                        ...
                        batsman2={
                            battingTeamPlayers.find((p) => p.user_id === nonStrikerId)
                                ? {
                                      id: nonStrikerId!,
                                      name: battingTeamPlayers.find((p) => p.user_id === nonStrikerId)!.user!.full_name,
                                      runs: 0,
                                      balls: 0,
                                      fours: 0,
                                      sixes: 0,
                    ...
                    <CurrentBowler
                        bowler={
                            bowlingTeamPlayers.find((p) => p.user_id === currentBowlerId)
                                ? {
                                      id: currentBowlerId!,
                                      name: bowlingTeamPlayers.find((p) => p.user_id === currentBowlerId)!.user!.full_name,
                                      overs: 0,
                                      maidens: 0,
                                      runs: 0,
                                      wickets: 0,
                    ...
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <p className="text-xs font-medium text-muted-foreground">{battingTeam?.name} — Batting</p>
                                ...
                                <div>
                                    <p className="text-xs font-medium text-muted-foreground">{bowlingTeam?.name} — Bowling</p>
                                    <div className="mt-1 flex flex-wrap gap-1">
                            ...
                            <Button variant="ghost" size="sm" className="mt-3 w-full" onClick={() => setShowAddPlayerDialog(true)}>
